...
2、spring 配置文件中开启 HystrixCommand 的 aop支持
...
```java
public Map<String, Object> zrPangu3ScoreQuery(Map<String, Object> req) throws Exception {
        //boolean timeOut = Boolean.parseBoolean(DefaultCommonConst.interTimeOut.get("hystrix_"+(String) req.get(BizConst.OPTYPE)));
        return new FuXiHystrixCommand("Thread_Pool_1",2000,5,1100,true){
            @Override
            protected Map<String, Object> run() throws Exception {
                String optType = (String) req.get(BizConst.OPTYPE);
                return dataSourceProcessMethod(req, getTableMap(optType), NOT_EMPTY, defaultCacheDays, dao, r2mClient,
                        logger, optType, false, true, false,
                        PlatformStatusCode.CURRENCY_CODE_OPERATION_SUCCESS, PlatformStatusCode.CURRENCY_CODE_NO_RESULT);
            }
        }.execute();
    }
```
